...
function invalidCommand(result) {
  return Q.fcall(function () {
    if (result.command.length > 0) {
      logger.info('   Invalid command: '.red + result.command);
    }
  });
}
...
function showHelpCommand(entryComponents) {
  ...
  return Q.fcall(function() {
    ...
    for (index in chosenCommand.details) {
      logger.info('   ' + chosenCommand.details[index]);
    }
...
function commandUsage(chosenCommand) {
  return Q.fcall(function () {
    logger.info('   Invalid Syntax for \'%s\''.red, chosenCommand.command);
    logger.info('   Usage: ' + chosenCommand.usage.green);
  });
}
      ...
      return dal.context.all()
        .then(function(contexts) {
          ...
          for (var context in contexts) {
            logger.info('   ' + contexts[context].domain);
          }
        })
        .catch(handleError);
    },
